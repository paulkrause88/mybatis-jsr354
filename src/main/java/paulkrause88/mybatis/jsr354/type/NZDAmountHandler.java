...
public class NZDAmountHandler extends BaseTypeHandler<MonetaryAmount> {
	
	private static final ThreadLocal<MonetaryAmountFactory<? extends MonetaryAmount>> NZD_FACTORIES = new ThreadLocal<>();
	
	static MonetaryAmount toNZD(BigDecimal big) {
		if (big == null) return null;
		MonetaryAmountFactory<? extends MonetaryAmount> factory = NZD_FACTORIES.get();
		if (factory == null) {
			factory = MonetaryAmounts.getAmountFactory();
			factory.setCurrency("NZD");
			NZD_FACTORIES.set(factory);
		}
	...
	public MonetaryAmount getNullableResult(ResultSet rs, String col) throws SQLException {
		return toNZD(rs.getBigDecimal(col));
	}
	...
	public MonetaryAmount getNullableResult(CallableStatement cs, int col) throws SQLException {
		return toNZD(cs.getBigDecimal(col));
	}
