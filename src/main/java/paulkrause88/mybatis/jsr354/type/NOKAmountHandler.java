...
public class NOKAmountHandler extends BaseTypeHandler<MonetaryAmount> {
	
	private static final ThreadLocal<MonetaryAmountFactory<? extends MonetaryAmount>> NOK_FACTORIES = new ThreadLocal<>();
	
	static MonetaryAmount toNOK(BigDecimal big) {
		if (big == null) return null;
		MonetaryAmountFactory<? extends MonetaryAmount> factory = NOK_FACTORIES.get();
		if (factory == null) {
			factory = MonetaryAmounts.getAmountFactory();
			factory.setCurrency("NOK");
			NOK_FACTORIES.set(factory);
		}
	...
	public MonetaryAmount getNullableResult(ResultSet rs, String col) throws SQLException {
		return toNOK(rs.getBigDecimal(col));
	}
	...
	public MonetaryAmount getNullableResult(CallableStatement cs, int col) throws SQLException {
		return toNOK(cs.getBigDecimal(col));
	}
