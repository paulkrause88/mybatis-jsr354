...
public class AUDAmountHandler extends BaseTypeHandler<MonetaryAmount> {
	
	private static final ThreadLocal<MonetaryAmountFactory<? extends MonetaryAmount>> AUD_FACTORIES = new ThreadLocal<>();
	
	static MonetaryAmount toAUD(BigDecimal big) {
		if (big == null) return null;
		MonetaryAmountFactory<? extends MonetaryAmount> factory = AUD_FACTORIES.get();
		if (factory == null) {
			factory = MonetaryAmounts.getAmountFactory();
			factory.setCurrency("AUD");
			AUD_FACTORIES.set(factory);
		}
	...
	public MonetaryAmount getNullableResult(ResultSet rs, String col) throws SQLException {
		return toAUD(rs.getBigDecimal(col));
	}
	...
	public MonetaryAmount getNullableResult(CallableStatement cs, int col) throws SQLException {
		return toAUD(cs.getBigDecimal(col));
	}
