...
public class IDRAmountHandler extends BaseTypeHandler<MonetaryAmount> {
	
	private static final ThreadLocal<MonetaryAmountFactory<? extends MonetaryAmount>> IDR_FACTORIES = new ThreadLocal<>();
	
	static MonetaryAmount toIDR(BigDecimal big) {
		if (big == null) return null;
		MonetaryAmountFactory<? extends MonetaryAmount> factory = IDR_FACTORIES.get();
		if (factory == null) {
			factory = MonetaryAmounts.getAmountFactory();
			factory.setCurrency("IDR");
			IDR_FACTORIES.set(factory);
		}
	...
	public MonetaryAmount getNullableResult(ResultSet rs, String col) throws SQLException {
		return toIDR(rs.getBigDecimal(col));
	}
	...
	public MonetaryAmount getNullableResult(CallableStatement cs, int col) throws SQLException {
		return toIDR(cs.getBigDecimal(col));
	}
