...
public class COPAmountHandler extends BaseTypeHandler<MonetaryAmount> {
	
	private static final ThreadLocal<MonetaryAmountFactory<? extends MonetaryAmount>> COP_FACTORIES = new ThreadLocal<>();
	
	static MonetaryAmount toCOP(BigDecimal big) {
		if (big == null) return null;
		MonetaryAmountFactory<? extends MonetaryAmount> factory = COP_FACTORIES.get();
		if (factory == null) {
			factory = MonetaryAmounts.getAmountFactory();
			factory.setCurrency("COP");
			COP_FACTORIES.set(factory);
		}
	...
	public MonetaryAmount getNullableResult(ResultSet rs, String col) throws SQLException {
		return toCOP(rs.getBigDecimal(col));
	}
	...
	public MonetaryAmount getNullableResult(CallableStatement cs, int col) throws SQLException {
		return toCOP(cs.getBigDecimal(col));
	}
